...
int main(int argc, char* argv[]) {
   ...
   for(i=0; i<dim; i++) {
    ...
    int **ps = malloc(dim*sizeof(int*));
    for(i = 0; i < dim; i++) {
    ps[i] = malloc(dim*sizeof(int));
    }
...
#pragma omp parallel num_threads(number_of_threads) firstprivate (ps) private (i,j,sum,pos,local_max) shared(max_sum,top,left,bottom,right)
{
      //#pragma omp for
        for (i=0; i<dim; i++) {
...
#ifdef _PRINT_INFO
    for(i=0; i<outmat_row_dim; i++) {
        for(j=0; j<outmat_col_dim; j++) {
            printf("%d\t", outmat[i][j]);
        }
        printf("\n");
    }
